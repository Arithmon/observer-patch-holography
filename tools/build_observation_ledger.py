...
def render(rows: list[dict]) -> str:
    ...
    lines.append(
        ...
        f" The JSON carries schema `{SCHEMA}`; the V3 in this page's title"
        " and file name names the V3 lane set its sections group rows by, not"
        " the schema."
    )
    lines.append("")
    lines.append(
        ...
        f" consumes. The row set, rungs, and lane assignments were fixed when"
        f" the ledger was established. The lane column names the V3 lane a row was composed"
        f" under; a section heading for a closed lane names the issues that own"
        f" its open work. The ledger records adequacy status. It is not itself"
    ...
    lines.append(
        "- **owed**: no qualifying receipt is committed; the owning lane, or"
        " the successors named for a closed lane, carries the obligation."
    )
    lines.append("")
    lines.append(
    ...
    for title, lanes in GROUPS:
        ...
        closed = [lane for lane in lanes if lane in CLOSED_LANE_SUCCESSORS]
        if closed:
            owners = ", ".join(
                _lane_link(successor)
                for lane in closed
                for successor in CLOSED_LANE_SUCCESSORS[lane]
            )
            lane_list += f", closed; open work: {owners}"
        lines.append("")
