...
declared and incomplete running/map packet evaluates to the target-free
running-chart coordinates \((80.330,\,91.119)\)~GeV; they are not source-emitted. Separate strict
...
The exterior census uses left Weyl fields \(Q,u^c,d^c,L,e^c\).
Four-component Yukawa terms instead use the charge-conjugated singlets
\(u_R,d_R,e_R\): \(\bar Q\widetilde H u_R\), \(\bar Q H d_R\), and
\(\bar L H e_R\). In \(q=6Y\) units their charges are respectively
\(-1-3+4=0\), \(-1+3-2=0\), and \(3+3-6=0\).
These identities and the singlet conjugation dictionary are kernel-checked
against the same exterior table. They fix notation and charge compatibility;
they do not determine Yukawa coefficients or a spacetime fermion action.
...
\(N_\Lambda=3.3126\ldots\times10^{122}\). Under the separately
...
\(N_\Lambda=3.3126\ldots\times10^{122}\), a propagated \(2.4\) to
...
\(2.5\times10^{-6}\) relative to the measured \(137.035999177(21)\). Its absolute residual \(3.3904\times10^{-4}\) exceeds the experimental standard uncertainty \(2.1\times10^{-8}\); this numerical proximity does not establish agreement at measured precision. The no-hadron packet
...
direct-top PDG row differs from the theorem coordinate by \(0.50~\mathrm{GeV}\),
or \(0.76\) combined standard deviations, because Q007TP and Q007TP4 are distinct
...
The target-free running-chart solve gives
\((80.330,\,91.119)\)~GeV. That running chart and the target-conditioned
closure chart described just above are numerically disjoint objects
under the same \(W/Z\) labels: the running chart admits no measured mass into
its arithmetic, and the closure chart solves its basis from the measured pair.
The Particle Data Group (PDG) comparison values are
...
The target-free running-chart coordinates carry no OPH theory covariance, and no physical
...
The finite-iteration hierarchy certificate records the interval
\[
...
\([-10.995768,-10.985284]\) for its declared finite map.
The exact implicit-scale attachment of
Theorem~\ref{thm:particle-implicit-scale-attachment} separately proves a
unique hierarchy zero in $I_U$ at the stored comparison decimal, with
residual derivative in $[-12,-10]$ and scale in
$[7,8]\times10^{-18}$. It retains the finite representation cutoffs.
On the public endpoint branch, at the CODATA-located comparison pixel \(P_C\),
\[
\begin{aligned}
...
On the target-free declared-map branch the two branch points give the target-free
running-chart coordinates
\[
\begin{aligned}
...
\(M_W\) and \(5.1\)~MeV on \(M_Z\). These are target-free
running-chart coordinates. The PDG mass targets use a mass-dependent-width Breit--Wigner convention, and
...
coordinate. The target-free running-chart coordinates are not physical observables and
...
\subsection{Target-conditioned closure-chart envelope}\label{ew-conditional-envelope}
...
\end{aligned}
\]
The recorded conditional central coordinates of this target-conditioned closure
chart are \(80.373315\) and \(91.193124\)~GeV, each inside its displayed
interval above.
The Higgs and top entries are target-conditioned comparisons on their declared mass scheme. The
\(W/Z\) entries are closure-chart coordinates. The PDG targets use mass-dependent-width Breit--Wigner
parameters, and complex-pole masses use another convention, so no physical coverage or pull is
assigned. At the endpoint pixel the two repair selections give closure-chart \(Z\) coordinates
...
diagnostics. No \(W/Z\) standard-deviation assignment is valid because the closure-chart coordinates,
PDG mass-dependent-width parameters, and complex poles use different conventions.
The \(W\) closure-chart spread is repair-selection freedom: the two candidate repair laws differ by about
...
The \(Z\) closure-chart row is the most sensitive adapter case. The empirical-closure pixel inherits
...
into the running couplings and re-evaluates the candidate \(W/Z\) closure chart. That branch shifts
...
inside the nonsingular domain of its imposed reciprocal-ray, affine-mean, fixed-shared-scale subfamily. The two-spread
...
An exact spectral construction and finite numerical witnesses distinguish admissibility from
selection. Given positive \((r,s)\), gaps \(sr/(1+r)\) and \(s/(1+r)\) determine a trace-free
spectrum; shifting it positive and setting
\(T=Q\,\operatorname{diag}(\sqrt{\mu})\,Q^{\dagger}\) gives
\(TT^{\dagger}=Q\,\operatorname{diag}(\mu)\,Q^{\dagger}\) with the prescribed gaps.
The numerical battery additionally imposes fixed gap and overlap floors.
If both gaps must exceed \(\delta>0\) (nominally \(10^{-12}\)), then necessarily \(s>2\delta\);
its passing kernels cannot cover every positive span. Shrinking a frame difference may improve
the defect-to-gap margin while violating the overlap floor. Twelve computed witnesses at
distinct ratios and spans pass all recorded checks, with invariant errors below \(10^{-9}\).
They establish numerical non-uniqueness on that grid, not universal surjectivity or a physical
source obstruction. The rounded template-context point is not an exact operating-point witness.

The three-scalar interface is exact on the nonsingular domain of its imposed reciprocal-ray
subfamily. At fixed shared scale \(g_{\mathrm{ch}}\), it factors through
\[
...
\(\rho=\ln(m_c/m_u)/\ln(m_t/m_c)\). Its domain further requires
\(p(r)=-r^3+4r^2+23r-2\ne0\), since
\(B_{ud}=(r+1)^2(r+5)/(2p(r))\). The polynomial has positive roots in
\((2/25,9/100)\) and \((71/10,36/5)\), by its endpoint signs and continuity.
The exact domain identity and both root-existence statements are machine-checked in
\texttt{QuarkMeanDomain}. The forward map is injective on this domain, and the executed round trip
...
Under the non-definability theorem's registry premise above, the stated source data select neither
...
weak-boson and Higgs numbers are running-chart and closure-chart coordinates.
...
B.\ M\"uller, A.\ Osika, M.\ Poneder, K.\ Xue, B.\ Cassie, P.\ Nguyen, J.\ Kim, D.\ Matscheko, J.\ Hill, W.\ T.\ Glynn, M.\ A.\ Visser, K.\ A.\ Anirudha, and B.\ de La Fourni\`ere,
...
B.\ M\"uller, A.\ Osika, K.\ Xue, B.\ Cassie, and B.\ de La Fourni\`ere,
...
\bibitem{ophEinstein}
B.\ M\"uller, A.\ Osika, M.\ Poneder, K.\ Xue, P.\ Nguyen, M.\ A.\ Visser, D.\ Matscheko, and B.\ de La Fourni\`ere,
