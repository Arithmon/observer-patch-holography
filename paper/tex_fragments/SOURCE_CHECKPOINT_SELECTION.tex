...
When the alphabet covers the graph edges and every alphabet move is
permitted on that graph, the composed theorem also ensures that every
selected history stays on the declared support. For arbitrary record maps
the publication predicate is classical; no general decision algorithm is
claimed. In the rational linear controls, exact elimination decides it.
