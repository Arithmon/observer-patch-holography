...
are kernel-checked in \texttt{GoldenSourceCountLimit}. Causal frontiers
have zero volume by the Fubini argument in \texttt{SourceCausalBoundary}.
The actual count measure converges on moving generated-order intervals;
the finite window remains part of the reference volume. Strict-pair counts
converge to the geometric product measure with the pair endpoint error
retained. The corresponding proofs are in \texttt{GoldenSourceCausalLimit}
and \texttt{GoldenSourcePairLimit}.

The actual geometric pair measure is evaluated by the radial integration
proof in \texttt{FlatDiamond\allowbreak PairIntegral}. For a timelike diamond contained
in the supplied window, its ordering fraction tends to $1/10$. This final
normalization is proved in \texttt{FlatDiamond\allowbreak Normalization}.
The explicit vertical Alexandrov error and general weighted-family limit
are proved in \texttt{SourceNet\allowbreak VolumeError}; the golden specialization is
in \texttt{GoldenSource\allowbreak VolumeLimit}. Extra routing events require another
count estimate. These theorems select no physical measure or clock.
