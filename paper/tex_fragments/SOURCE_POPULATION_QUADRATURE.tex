...
are kernel-checked in \texttt{GoldenSourceCountLimit}.
\texttt{SourceCausalBoundary} proves causal-diamond frontier nullity;
\texttt{GoldenSourceCausalLimit} transfers the actual count measure to
moving generated-order intervals, with the finite window retained in the
reference volume. \texttt{GoldenSourcePairLimit} proves strict-pair
convergence to the geometric product measure, retaining the pair endpoint
error. \texttt{FlatDiamondPairIntegral} evaluates the actual geometric
pair measure; \texttt{FlatDiamondNormalization} proves the $1/10$ limit
for timelike diamonds contained in the supplied window.
\texttt{SourceNetVolumeError} proves the explicit vertical Alexandrov
error, and \texttt{GoldenSourceVolumeLimit} specializes it to the golden
weighted-volume and raw-count limits. Extra routing events require another
