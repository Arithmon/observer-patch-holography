...
The proposed record-gluing law RG (Section~\ref{sec:source-record-gluing})
connects response directions to classical record transport. It has distinct information, geometry and implementation
...
and nonnegative elapsed time converging to the original time. Local operations
and waits have zero displacement. A local primitive costs
...
The quadratic form inherits the latency symmetry by dividing
$\ell(sRv)^2=\ell(sv)^2$ by $s^2$ and taking $s\to0$.
...
a nonempty finite state family. Choose the local references as restrictions
of the same faithful joint tracial state. These compatible references and an injective
...
Compatibility matters: on a shared classical bit with two equal-weight
references $(1/2,1/2)$ and $(3/4,1/4)$, both faithful and tracial, the summed
entropy derivative at probability $1/2$ is $-\log3$, so the joint reference
does not minimize that incompatible-reference problem.

There is also a finite process-to-state connection. For $g$ gates on $R$
bits, $n$ of them independent input bits, let $D=R(g+1)$ and admit all
$2^D$ ambient histories. Initial blanks and actual gate equations define
the feasible family by linear indicator constraints. Each input $x$ has
exactly one feasible history $h(x)$. A faithful input prior $p(x)$ with fair
references on all other history coordinates gives
$Q(h(x))=p(x)2^{-(D-n)}$. Hence every feasible $P$ satisfies
\[
 D(P\Vert Q)=D(P_{\mathrm{input}}\Vert p)+(D-n)\log2.
\]
With the full joint history observer as the injective scored cover, the
unique minimum is $P_*(h(x))=p(x)$, exactly the executed circuit ensemble.
Feasibility uses actual transitions, not success at the target truth table:
a defective program retains its defective histories with positive weight.
This is an explicitly classical finite score problem. Additional scored
marginals, the program and its physical clock are not selected by this
identity. The native all-level source obligations remain separate.
...
For inner menus, the same cube is included when
$\eta_q<1-\sqrt3/2$, giving the asymptotic bounds; the all-$n$ constants
above concern the full-radius experiment.
...
The compact receipt independently verifies the full named catalog and replays
263 finite circuits, including 7,880 localized interventions and 40 paired
intermediate-writer comparisons, with every reference gate and work register
counted. Coordinated circuit/table substitutions and cached-writer substitutions
fail verification. Exact algebra controls cover all 1,728 basis units of the copying
