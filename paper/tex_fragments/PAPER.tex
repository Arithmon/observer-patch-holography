% Shared recovered-core section. Two papers include this file and select
% their branch text with \OPHEinsteinDerivationPaper and
% \OPHStandardModelDerivationPaper: the spacetime and Einstein-dynamics
% derivation, and the Standard Model gauge-structure derivation. The file
...
hyperboloid. Conservative Fibonacci-record populations, complete reads inside
a shrinking metric radius, and a declared model clock give a separate flat
$1+3$ causal and count-volume limit. This supplied law does not identify a
physical source. The exact-embedding route to physical spacetime requires
source-selected placements with two-way cone/order agreement; the controlled
route permits uniformly vanishing causal discrepancy. Both require compatible
physical clocks, count calibration and continuum geometry. Smooth Einstein promotion further
...
Specified boundary field theory & Observer patches with typed overlaps \\
Conformal dual in specified constructions & Patch algebras plus geometric and physical premises \\
Negative \(\Lambda\) in AdS & Positive \(\Lambda\) on the declared de Sitter branch \\
\end{longtable}
}
...
\(N_\Lambda=3.3126\ldots\times10^{122}\). The conditional
...
parameters; complex-pole masses are a different convention. The running-chart coordinates
are not commensurate with either physical mass definition and carry no near-hit evidence.
