...
**Convention.** This note takes $`\omega`$ to be the primitive cube root of unity, with $`\omega^2 = -1 - \omega`$. The certified A5 code in this repository (`code/a5_closure/flux_defect_criterion_certificate.py`) works with the primitive sixth root $`\zeta_6 = e^{\pi i/3} = 1 + \omega = -\omega^2`$, which satisfies $`\zeta_6^2 = \zeta_6 - 1`$ and gives $`N(a + b\zeta_6) = a^2 + ab + b^2`$ (Gram matrix $`\begin{pmatrix}2&1\\1&2\end{pmatrix}`$ for twice the norm). Both generate the same ring, $`\mathbb{Z}[\omega] = \mathbb{Z}[\zeta_6]`$, and the substitution $`\zeta_6 = 1 + \omega`$ translates between the two conventions.
...
This bridge requires more than a single congruence condition. The Coxeter–Todd lattice $`K_{12}`$ is constructed from $`\mathbb{Z}[\omega]^6`$ using the **hexacode** as glue. The glue lives over the inert prime 2: since $`2 \equiv 2 \pmod 3`$, the prime 2 stays prime in $`\mathbb{Z}[\omega]`$ and $`\mathbb{Z}[\omega]/(2) \cong \mathbb{F}_4`$.

**Definition 3.1** (Hexacode). The hexacode $`\mathcal{H}_6`$ is the $`[6, 3, 4]_4`$ linear code over $`\mathbb{F}_4 = \{0, 1, \bar\omega, \bar\omega^2\}`$ generated by the rows of

$$\begin{pmatrix} 1&0&0&1&\bar\omega^2&\bar\omega \\ 0&1&0&1&\bar\omega&\bar\omega^2 \\ 0&0&1&1&1&1 \end{pmatrix},$$

where $`\bar\omega`$ is the image of $`\omega`$ in $`\mathbb{Z}[\omega]/(2)`$. It has 64 codewords, and every nonzero codeword has weight at least 4, so it meets the Singleton bound. Conway and Sloane [1] treat the hexacode and its role in lattice constructions.

**Definition 3.2** (Coxeter–Todd lattice via the hexacode). The Coxeter–Todd lattice is the set of $`(z_1, \ldots, z_6) \in \mathbb{Z}[\omega]^6`$ such that the residue vector

$$(z_1 \bmod 2, \ldots, z_6 \bmod 2) \in \mathcal{H}_6,$$

where the reduction $`\mathbb{Z}[\omega]^6 \to \mathbb{F}_4^6`$ is applied componentwise, with the real inner product $`\mathrm{Re}\sum_i z_i \bar w_i`$.

The index of $`K_{12}`$ in $`\mathbb{Z}[\omega]^6`$ is $`|\mathbb{F}_4^6 / \mathcal{H}_6| = 4^{3} = 64`$, so its determinant as a real lattice is $`(3/4)^6 \cdot 64^2 = 729`$. Vectors of norm below 4 would need a codeword of weight below 4 or a coordinate in $`2\mathbb{Z}[\omega]`$ of norm below 4, and neither exists. The minimal vectors are the $`45 \cdot 2^4 = 720`$ unit lifts of the weight-4 codewords plus the $`6 \cdot 6 = 36`$ vectors $`2u\,e_i`$ with $`u`$ a unit, 756 in all. A direct enumeration of the vectors of norm at most 4 confirms these counts.
...
**Remark 3.4.** Reduction modulo $`\theta = 1 - \omega`$, which lands in $`\mathbb{Z}[\omega]/(\theta) \cong \mathbb{F}_3`$, cannot supply this glue. A ternary $`[6, 3]`$ code has minimum distance at most 3: a $`[6, 3, 4]_3`$ code would be MDS, and a ternary MDS code of dimension at least two has length at most four. Every such construction therefore contains vectors of norm 3 and is not $`K_{12}`$.

**Remark 3.5.** The three kinds of prime from Bridge 1 all appear here. The prime 3 ramifies as $`3 = -\omega^2 \theta^2`$, with $`N(\theta) = 3`$ the same "3" as in the conductor of $`\chi_{-3}`$; the inert prime 2 carries the hexacode glue; and the split primes play no role in this bridge.

---

## 4. Bridge 3: From $`K_{12}`$ to the Leech Lattice $`\Lambda_{24}`$

The third bridge requires the **binary** Golay code.
...
| 2 | $`K_{12}`$ | $`B_2`$ | Hexacode over $`\mathbb{F}_4 \cong \mathbb{Z}[\omega]/(2)`$ |
...
2. **Bridge 2** (Coding theory + lattice theory): The hexacode $`[6,3,4]_4`$ as algebraic glue inside $`\mathbb{Z}[\omega]^6`$, read modulo the inert prime 2.
...
The arithmetic of $`(\mathbb{Z}/6\mathbb{Z})^\times = \{1, 5\}`$ enters the foundational ancestry of four structures: the Eisenstein integers (via the splitting dichotomy), the Coxeter–Todd lattice (via the hexacode), the Leech lattice (via the binary Golay code), and the Monster sporadic simple group (via the Moonshine module). The Conway groups appear along the way as automorphism groups of the Leech lattice; they are a by-product of Bridge 3, not a link of the chain.
...
The value of understanding this chain is the precise mathematical map it provides: a tiny arithmetic object, the units modulo 6, indexes the first link of a sequence that culminates, four non-trivial bridges later, in the largest sporadic simple group. The role of $`(\mathbb{Z}/6\mathbb{Z})^\times`$ throughout is that of an indexing label, not a generator.
