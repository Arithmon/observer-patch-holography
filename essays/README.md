...
correct, and a methodology report describing how the corpus behind them was produced.
Each entry has a standalone LaTeX source and the PDF built from it.
...
Each essay was written entirely by an AI model, credited as Claude Fable, from a single
...
Figures are in `figures/` as PDF. Each source compiles on its own with
