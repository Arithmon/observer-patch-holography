...
| Scheduler-class capacity split | `SchedulerClass.all_scheduler_defect_cut`; `capacity_split_separation`; `cross_hit_requires_shared_spend`; `class_does_not_force_almostSure`; `fine_layer2_eq_layer3_of_class`; `coarse_endpoint_capacity_split_receipt` | Obstruction bounds quantified over the declared admissible class and its self-update-only and network-update-only sub-classes on the two-register carrier: the defect fiber for the whole class, the cross fiber and coarse layer 3 for the sub-classes, with explicit escape witnesses and a fine-fixture collapse of layer 2 onto layer 3. | No scheduler selection, rate, hitting time, or generality beyond the declared two-register carrier and capacity-split reading; the locality and capacity clauses are typed but not load-bearing. |
