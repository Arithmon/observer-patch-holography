...
- `Geometry/GoldenSourceAssignment.lean`: Fibonacci arithmetic identifies the exact fractional-part population with a permuted equal-cell assignment, proves nonwrapping and injectivity, the q³ image size, measurable partition, exact Lebesgue cell masses, and a Bochner-integral quadrature bound with convergence. The two-cell-width assignment bound has a formal counterexample to replacing it by one cell width. The source population, read law and physical clock remain supplied; null-boundary causal-interval and pair-count specialization are separate.
