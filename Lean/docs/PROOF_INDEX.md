...
- Source-family count/volume and ordering-fraction limits (#782): kernel-clean.
  ...
  raw-count convergence. The existing golden quadrature estimate is consumed.
  ...
  See [`SOURCE_COUNT_LIMIT_AUDIT.md`](SOURCE_COUNT_LIMIT_AUDIT.md) for the
  acceptance mapping and exact hypotheses.
...
- `Geometry/FlatDiamondError.lean`: the cubic shell modulus and the exact inclusive temporal Riemann error πc³T³Delta/2. Paper: `SOURCE_NET_CAUSAL_LIMIT.tex`, proof of `prop:source-net-alexandrov-volume`.
...
- `Geometry/SourceCountLimitAxiomAudit.lean`: an enforced transitive axiom allowlist and receipts for all 142 new public theorems plus the three consumed golden quadrature and injectivity interfaces. Only propext, Classical.choice and Quot.sound are permitted; negative controls check rejection of admissions and compiler-trusted reductions.
