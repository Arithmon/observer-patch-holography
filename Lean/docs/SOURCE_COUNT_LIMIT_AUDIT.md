...
| --- | --- | --- |
| Weighted Alexandrov volume with the explicit error | `SourceNetVolumeError.weighted_alexandrov_error`; `GoldenSourceVolumeLimit.golden_weighted_error`, `alignedVolume_tendsto`, `alignedCount_tendsto` | A convex h-covered population, all-neighbour read radius a>2h, positive supplied c, finite measurable cells with almost-everywhere disjointness/coverage and assignment error H, and the interior buffer. The golden specialization derives these spatial facts from the actual fractional golden orbit. |
...
checks every one of its 141 new public theorems with `assert_no_sorry` and
`#print axioms`, plus the existing golden quadrature and site-injectivity
interfaces. The permitted transitive axiom set is exactly a subset of
`{propext, Classical.choice, Quot.sound}`. The pinned environment is Lean
