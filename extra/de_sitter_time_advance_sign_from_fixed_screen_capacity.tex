...
\paragraph{A concrete stationary repair control.}
The balanced conservative twelve-port family illustrates the domain
distinction with its actual local repair law. On a connected weighted
port graph, let \(z=n-(r/p)\mathbf1\) and
\(\kappa=r(p-r)/[p(p-1)]\). Fair endpoint exchange at each Poisson
attempt has density drift \(Gz=-Lz/2\), uniform stationary occupancy,
and \(\operatorname{Cov}(z(t),z(0))=\kappa e^{-|t|L/2}\Pi\).
At fixed graph, its integrated fluctuation record has covariance
\(4\kappa L^+\). This derives a graph Green covariance from local port
readback and repair histories; it neither deforms sector dimensions nor
identifies the record with a horizon shock.

For a stationary reversible continuous-time chain and centered
current-state observable \(f\), spectral resolution gives
\[
 \langle f,e^{tG}f\rangle_\pi=\sum_j a_j^2e^{-\gamma_jt},
 \qquad \gamma_j\ge0.
\]
It is nonnegative and nonincreasing. A nondecreasing global time change
preserves that property. The discrete-time version requires nonnegative
eigenvalues, satisfied by a lazy kernel \((I+K)/2\); the nonlazy
two-state flip instead gives \((-1)^m\). These are controls for a
specified classical autocorrelation, not constraints on an out-of-time-
ordered quantum correlator or an enlarged sector-dimension dynamics.
Exact finite witnesses and source-pinned operator controls are archived
under \path{evidence/observer_dynamics_20260925/sim-analysis/codex/}.

The inverse covariance also retains geometric anisotropy:
\([L^{-1},U]=-L^{-1}[L,U]L^{-1}\) on the mean-zero subspace.
The native level-four seven-mode band has \(14.642\%\) splitting,
whereas an independently supplied round-metric control has \(0.01561\%\).
Band averages or a \(3,5,3\) multiplicity pattern therefore do not
by themselves supply the rotational covariance assumed by a round
horizon attachment. The conditional shock-spectrum identification must
keep its metric and representation premises.
