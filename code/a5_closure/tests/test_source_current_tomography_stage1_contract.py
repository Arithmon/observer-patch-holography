...
class SourceCurrentTomographyStage1Tests(unittest.TestCase):
    ...
    def test_algebraic_capacity_does_not_claim_reconstructed_native_rank(self) -> None:
        gap = self.payload["algebraic_capacity_gap"]
        self.assertEqual(gap["algebraic_tangent_dimension_deficit"], 8)
        self.assertFalse(gap["native_tangent_rank_reconstructed"])
        self.assertNotIn("independent_tangent_directions_missing", gap)
