...
class SourceCurrentOrderSensitiveInventoryTests(unittest.TestCase):
    ...
    def test_committed_inventory_replays_and_verifies_independently(self) -> None:
        ...
        self.assertEqual(report["candidate_count"], 33)
    ...
    def test_decisive_near_candidates_remain_separated(self) -> None:
        ...
        routing_summary = self.committed["summary"]["routed_read_candidate_summary"]
        self.assertEqual(routing_summary["candidate_id"], "source_read_routing_full_family")
    ...
    def test_audit_scope_extension_is_explicit_and_bound(self) -> None:
        extension = self.committed["audit_scope"]["directories_added_since_initial_inventory"]
    ...
    def test_rehashed_scope_extension_omission_is_rejected(self) -> None:
        ...
        mutant["audit_scope"]["directories_added_since_initial_inventory"].pop()
    ...
    def test_recorded_branch_inverse_cannot_be_promoted_to_native_current(self) -> None:
        mutant = copy.deepcopy(self.committed)
        row = next(
            row for row in mutant["candidates"]
            if row["candidate_id"] == "declared_flagged_recovery_extensions"
        )
        self.assertTrue(row["properties"]["reversible"])
        self.assertFalse(row["properties"]["source_native"])
        self.assertFalse(row["properties"]["twelve_reversible_perturbation_families"])
        row["properties"]["source_native"] = True
        row["qualification_failures"].remove("source_native")
        path = self.write_mutant(mutant)
        with self.assertRaises(independent.VerificationError):
            independent.verify(path, REPO_ROOT)

    def test_new_native_and_selection_packages_fail_closed_on_semantic_drift(self) -> None:
    ...
    def test_renamed_audited_source_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.copy_required_repository_tree(root)
            target = root / "Lean/Dynamics/CenterSpectral.lean"
            target.rename(target.with_name("RenamedCenterSpectral.lean"))
            with self.assertRaisesRegex(independent.VerificationError, "AUDITED_PATH_DRIFT"):
                independent.verify(producer.INVENTORY_PATH, root)
