# Source-current tomography: Stage 2 inventory
...
both mixed composition orders of twelve perturbation families.

This statement applies only to the explicitly enumerated candidates and
reviewed source scope. It is not a no-go theorem for other source data, does
not weaken the abstract forced Lie-type
theorem, and does not reject the conditional charged-double-triplet fixture.
The fixture was not used as a reconstruction oracle.

## Qualification rule

A candidate qualifies only if one packet supplies all nine fields below. Facts
from distinct packets are not composited.

1. source-native;
2. port-indexed;
3. reversible;
4. both composition orders recorded;
5. raw histories serialized;
6. target-free;
7. same twelve-port carrier;
...
The registered icosahedral adjacency-response candidate has exact
response-word algebra has dimension four and zero nonzero commutators; it
serializes no raw runtime histories. The logged W12/refinement candidate is the
...
The full-family routed read compiler is
...
histories conditional on a declared connected scalar pair-mean grammar and
read interface. Those means can depend on order and are destructive; they are
not the registered all-port driver's disjoint matching. Neither successful
conditioning nor a supplied demand law derives twelve reversible native
perturbation families.

`finite_state_process_selection` gives finite noncommutative A3 support
results and exact supplied state/channel controls. The full feasible source
grammar and the identification of a required-read failure effect are premises.
The channel controls solve a separate declared process-state problem; a
selected state does not identify its temporal instrument.

`registered_operation_read_histories` retains seven actual all-port source
cases, one prescribed icosahedral Laplacian unitary, and local ledger reads. Its matching
repairs commute, are idempotent and erase differences. Coordinate probes and a
time-translated probe generate the full noncommutative algebra M12(C), but
noninvertible probes and algebraic closure are not reversible perturbation
flows. This full probe algebra is separate from the four-dimensional
icosahedral adjacency-response algebra above. Extending these histories does
not supply spatial refinement.

`declared_flagged_recovery_extensions` preserves a positive conditional
result: each fixed recorded swap word is unitary and invertible. Retaining
the quantum state and 6N branch bits gives a recovery map on the image of the
flagged channel, not a two-sided automorphism of its enlarged output algebra.
The deterministic archive requires 5N+1 added real coordinates and global
recovery on its constrained normalized image; this is not a finite-bit or
local decoder. Unconditional twirl and dephased repair remain irreversible.
The flags, archive and channel are supplied extensions, not source-selected
twelve-family perturbation data.

The prior candidate families retain the following scope.
...
Positive tomography requires a separate independently reviewed producer with:

- exactly twelve reversible source-native families `P_0,...,P_11`;
- `p`-then-`q` and `q`-then-`p` histories for all 66 distinct unordered port
  pairs (132 ordered compositions);
- canonical raw-history custody with provenance and tamper-evident hashes;
- a common W12 carrier, orientation, normalization, source family, and
  refinement stage/tower;
- an inverse or two-sided-law witness for every family, excluding reset,
  averaging, measurement, and strict-descent repair;
- a source firewall excluding current fixtures, gauge/particle labels,
  measurement targets, and downstream selectors.

The enumerated packets authorize no ordered bracket reconstruction,
Jacobi/compactness selection, same-current holonomy, or physical comparison.
