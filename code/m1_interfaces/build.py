...
def graph(config,family,topology):
    masks,cuts,reads,action=model.graph_cuts(config,family,topology)
    q=config[0]
...
def build():
    return dict(schema='oph-m1-interfaces-v2',sources=pins(),parent_claims=parent_claims(),
