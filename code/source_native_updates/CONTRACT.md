...
records must all be prepared in advance, and cleanup has only been proved for
a four-cell bus. Pursue the user's larger M1 objective without equating these
sub-results with a full M1 derivation.
...
5. A supported two-schedule control showing that the candidate local mean
   ...
   countermodel. State precisely what remains open for source selection.
...
JSON must be rejected even if their digests are recomputed. Every new theorem
...
and the derivation of the metric-neighbour read law remain required for M1.
None of #777/#779/#740 or M1 can be closed by this package alone.
