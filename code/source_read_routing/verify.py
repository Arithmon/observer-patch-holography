...
def check_specification(packet=None):
    ...
    require(packet.get("exit") == {
        ...
        "acceptance_matrix":"code/source_read_routing/CONTRACT.md"},
...
def verify(receipt_path, binary, work, *, input_path=None, raw_rows=None):
    check_specification()
    ...
    info = packet["inputs"]
    input_path = folder / f"q{info['q']}.input" if input_path is None else Path(input_path)
    q, n, rounds, centre, indptr, indices = inspect_input(input_path, info)
    check_resources(packet)
    def blocks():
        if raw_rows is not None:
            yield from raw_rows
        elif "segments" in packet["tape"]:
    ...
    if q == 13 and variant == "baseline":
        from support_comparison import check_q13_projection
        actual["support_wiring_comparison"] = check_q13_projection(info, indptr, indices, logical)
