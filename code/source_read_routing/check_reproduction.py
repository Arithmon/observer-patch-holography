...
def reproduce(producer, verifier, work):
    ...
    for variant in ("baseline", "source", "branch", "scratch"):
        ...
        regenerated = regenerate(HERE / "controls" / result.name, producer, verifier,
                                 work / "regenerated")
        require(regenerated["decoded_sha256"] == replay["decoded_sha256"],
                "stream regeneration differs from materialized replay")
