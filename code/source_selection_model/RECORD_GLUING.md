...
The shrinking null shell has zero four-dimensional Lebesgue measure.
...
native causal ancestry restricted to the distinguished commits. Faithful
lowering gives P_q subset N_q. The path bound gives N_q subset C_c at the
...
radius or read menu. Two layers, the baseline and every single-site +1
intervention replay 2,683,568 primitive events. The independent checker
...
the covering-path theorem, value/intervention preservation under primitive
lowering, event-count retention, q^3 cardinality and causal-margin stability.
