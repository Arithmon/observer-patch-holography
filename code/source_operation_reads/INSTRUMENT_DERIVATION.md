...
convention. All seven retained primitive/observer cases and the original
quantum controls have identical numerical payloads across the extraction
interfaces. The additional preparations below enter at the post-unitary
operation boundary; they are
interventions, not purported Gaussian-seed outputs.

## 1. Two exact obstructions to promoting the registered rule
...
holds for the real-valued threshold rule; it is not an objection
...
||B-I||_infinity<=4*eta=5/8192<1/2. The Neumann series makes B invertible with
...
They neither show that the registered source keeps those records nor select
...
desired read law: the native threshold and lift are not exact channels
...
family. Selection within that family requires a realization of the complete A1
response and endogenous A2 data. Nothing here supplies
