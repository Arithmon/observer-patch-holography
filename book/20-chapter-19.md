...
Max Born wrote the odds rule down in 1926, in a paper on collisions, putting it in the body of the text as the amplitude and correcting it to the square of the amplitude in a footnote. That footnote is the **Born rule**, one of the three axioms chapter one said quantum mechanics arrived with. Textbooks teach the squaring as a postulate.
...
Consider a constructed table with eight settings, a pair of exclusive answers at each, and an expected frequency for each answer. The entries are chosen to fit a specified state; no instrument was run to produce them. Then go looking for another odds rule that fits the same table, with the trace formula given no privileges in the search.

One exists, and it is explicit. It reproduces every prescribed frequency exactly, as an identity between whole-number ratios rather than as a fit. Every question it scores lands between zero and one. The question with the certain answer gets one. A question and its opposite get numbers that add to one. And on every sum of exclusive questions that the table contains, it adds up correctly.

As a rule over all weighted questions, it belongs to no state. No density matrix reproduces all its assigned weights; matching the finite table does not fix the weights outside it.

Where it breaks is a pair of questions outside the table. Take the question this rule scores at 143/512, blunt enough that two copies of it side by side are again a legal question. Adding up demands that the pair score twice 143/512, so double it: 143/256. The rule scores the pair 35/64, which over the same denominator is 140/256. The gap is exactly three parts in two hundred and fifty-six. That single pair is enough to prove the rule belongs to nobody's state.

That locates the boundary. Additivity restricted to the sums in the table amounts to nothing more than the numbers in each setting adding to one. A demand that weak leaves the fake alive. The work is done by requiring the adding-up to hold for every compatible sum of weighted questions, including those outside the table. Impose that, and the same prescribed frequencies leave exactly one state standing, without a continuity assumption and without any appeal to long runs.

The finite table therefore does not select the probability rule. The theorem assumes that the weight of a question is independent of the setting in which it appears, and that combining compatible alternatives adds their weights. For a physical instrument, those assumptions require an operational account of which procedures ask the same question and how alternatives are combined. Agreement on overlapping records alone does not provide that account. Common preparation, the phase-sensitive operations and public readout also need physical implementations; chosen counts supply none of them.
