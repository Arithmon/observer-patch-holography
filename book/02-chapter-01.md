...
The answer is unflattering. Nobody's cleverness is the problem. Every attempt, including the ones that worked, began by assuming most of the answer.
...
Fermi's objection is arithmetic. There is a difference between a description that fits the data and a description that predicts it. The difference is countable. Suppose you have ten measurements, taken at ten different settings, and you draw a curve through them. If the curve has ten adjustable knobs, as a polynomial with ten coefficients does, the fact that it passes through all ten points tells you nothing, because such a curve passes through any ten points you like. If it has one knob and goes through all ten anyway, you have found something.
...
Look at what survived. The theory arrived with a space of states, a rule for how they evolve, and a rule for turning them into probabilities. All three were written down as axioms, and Born's has never been wrong. OPH gets it from the agreement that overlapping observers owe each other.

**The Standard Model.** Assembled between about 1954 and 1973, with a record of successful predictions nothing else in science comes near. Its structure arrived from outside, item by item. The symmetry group is a copy of what the detectors were reporting. The three families are a headcount of what those detectors found. Nineteen of its numbers are laboratory readings, or about twenty-six once neutrino masses are included, plus Newton's constant and the cosmological constant from the other theory entirely.
...
**The seam.** The two theories do not fit, and the way they fail is one you have just been given the equipment to read. It is a parameter-counting failure.
...
Try to force it anyway, quantizing gravity the way everything else was quantized, and you get a theory that produces infinities. Infinities on their own are survivable. The Standard Model has them and disposes of them by absorbing each one into a measured parameter, which is what those nineteen numbers are partly doing. For gravity the absorption never terminates. Every order of the calculation produces a fresh infinity requiring a fresh parameter. At low energies the extra terms are tiny, and [Donoghue's calculation](https://arxiv.org/abs/gr-qc/9405057) shows that the long-distance quantum corrections can be computed anyway. Push toward high energies and the list of parameters never closes. A theory with infinitely many free parameters fits anything and forbids nothing. Fermi's objection to four, taken to its limit.
...
This is the closest thing mainstream physics has to a granular world. It repays a careful look, because a finite world and a finite calculation are different animals.

The lattice is scaffolding. Its spacing is varied from run to run and taken to zero at the end, and the whole art of the subject is estimating and controlling the error a finite spacing makes. The action and its parameters, quark masses included, go in as inputs. Lattice quantum chromodynamics is a discrete method for computing the consequences of a continuous theory in a continuous spacetime. It is a triumph, and it surrendered nothing.

**String theory.** Stop treating particles as points and treat them as extended objects, and you get a different framework for quantum gravity. In 1984 [Michael Green and John Schwarz](https://doi.org/10.1016/0370-2693(84)91565-X) showed that the gauge and gravitational anomalies of a specific superstring theory cancel, and the field changed direction almost overnight. The price was extra dimensions, six or seven of them, curled up small enough to have escaped notice. In 1995 Edward Witten pointed out that the five competing versions of the theory that people had been arguing about were one theory viewed from five angles, in eleven dimensions, and named the eleven-dimensional thing M-theory without saying what the M stood for.
...
The program has real derivations to its name. [Andrew Strominger and Cumrun Vafa](https://arxiv.org/abs/hep-th/9601029) counted quantum states and recovered the area-entropy relation for a class of extremal black holes. [Juan Maldacena's correspondence](https://arxiv.org/abs/hep-th/9711200) relates specified quantum field theories to strings and gravity in anti-de Sitter settings, so that a gravitational bulk is reconstructed through a different description. That other description is a field theory on a fixed boundary spacetime.
...
**Background-independent quantum gravity.** This program took on the hardest thing on the list, and it deserves to be named properly for that. General relativity says the geometry of spacetime is dynamical: it bends, it responds, it is part of the physics rather than the setting for it. So a quantum theory of gravity should not begin by fixing a geometry and doing quantum mechanics on top. Starting in the 1980s, loop quantum gravity set out to quantize gravity without ever choosing a background metric, and its quantum geometry comes with discrete spectra for area and volume. [Abhay Ashtekar and Jerzy Lewandowski's account](https://arxiv.org/abs/gr-qc/0404018) explains the construction and its starting structures. It succeeded in giving up the fixed metric, which nothing else on this list managed.
...
None of them gave up the arena, the set of events with a place for things to happen in. Read the log again and the first half is a sequence of assumptions being removed, one per revolution, at forty-year intervals. The second half is a sequence of structures being added on top of an assumption that nobody touched.

## The rule

There is a fifth program. It sits outside that tally because it did the one thing the other four would not.

On 14 April 2020 Stephen Wolfram announced a project that had been running quietly for years, with much of the technical work carried by a young mathematician named Jonathan Gorard. Its starting object is a **hypergraph**, a collection of relations among elements that are nothing in particular, carrying no positions, distances or dimension. One rewriting rule says that wherever some pattern of relations appears, replace it with another pattern. Apply it wherever it matches, and keep applying it.

Everything else is supposed to come out of that. Space is what the graph looks like from far enough away. The number of dimensions is a property you measure on the graph rather than a number entered in advance. Time is the updating. What an observer can know is the causal graph, the record of which updates fed which. The sharpest part of the [project's technical account](https://www.wolframphysics.org/technical-introduction/potential-relation-to-physics/basic-concepts/) concerns order. If the causal graph comes out the same whatever order the updates are applied in, a condition called causal invariance, then two observers who disagree about the order still agree about the causes, and special relativity is what that agreement looks like from inside. Keep the orders running in parallel rather than collapsing them and the branching has the shape of quantum mechanics. Its [treatment of observers](https://wolframinstitute.org/output/observer-theory) places them inside the evolving system, with bounded computational resources.

Put that beside the four. Every program in the previous section defended the arena. This one deleted it. Every argument in this book is downstream of that move being available at all. The debt is not a courtesy.

Then look at what went onto the board in its place.

The rule. One rule, drawn from a space of candidates that is not small, wanted because it produces this universe rather than one of the others. The method for finding it is to search: enumerate simple rules, run them, inspect what comes out. Nothing inside the framework prefers any candidate to any other. The later proposal, that every rule runs and an observer samples the whole collection of them, is a way of living with the difficulty rather than a way of ending it.
...
Almost blank. One object survives. It is a strange thing to find at the bottom of physics, because it is neither matter nor geometry. It is finite. It can read a part of itself and a part of whatever it is next to. It keeps a record of what it read. And it has a way of settling the disagreement when its record and its neighbor's record fail to match.
