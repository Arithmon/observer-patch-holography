...
Causality is the partial order. A timeline is a total order. You can have the first without the second. The first is the one the world hands out, built from records of which read depended on which write. The second is available to anybody willing to break ties, which can be done more than one way.
