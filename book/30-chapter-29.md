...
What a corner carries is an amplitude, and chapter nineteen's rule for turning an amplitude into a measurable quantity is to square it. Each eigenvalue is therefore the square root of its corner's mass, up to one overall scale that fixes the units and cancels out of any ratio. The square root of a mass is a positive number, so the only responses describing anything that exists are the ones whose three scale factors all come out positive.
...
The wiring fixes the electric charges to every decimal anyone has measured. It fixes the number of colors at three; the number of families, three, is a declared input of the matter table. It carries the balance at the corners of one oriented face, equal weight for the two ways they can respond, and that balance is Koide's two thirds; the equal weight is the condition Koide published, and a derivation of it from the wiring alone is work in progress. Handed the electron and the muon and that balance, it pins the tau inside 72 electron-volts and prints the value at which the balance dies. It does not hand over the angle by which the weak interaction turns a down quark into a strange quark: 20.9052 against 13.0029 proves that the angle is no axis angle of this solid. Had 13.0029 turned up anywhere on that list, the charges and the family count would have been worth less, because a solid that yields whatever angle is asked of it has yielded nothing.
