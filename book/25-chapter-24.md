...
A field can be calculated on those same prepared addresses. Nearest neighbors exchange a scalar field through one specified action, which also defines its quantum states and detectors. A disturbance confined to one slab produces a response at a detector in a separate slab; the certified error stays smaller than that response throughout the observation window. The comparison is with a massive field in a cube with fixed walls. Protected addresses are stored separately from changing field values. The continuum comparison also covers local discrete time updates, including their extra error. Reading the closest update keeps the signal resolved across the full observation window. Before a continuum signal can cross the separation, the induced response tends to zero with refinement.

A coarser execution records every field read and write across 64 sites. Its quantum counterpart uses the same action, its vacuum and a coherent preparation. The dependency graph recovers successive configurations without reading timestamp labels. For a specified family of actions, two independent directions of field change determine the positive duration of each interval, even when equal durations are not assumed. For a common history stationary under that action, bounds on record error give bounds on elapsed action time. These timing bounds can be included in the quantum detector comparison: 15 of the 21 predicted responses at recorded updates exceed the combined error throughout their inferred time intervals. This comparison keeps the action and quantum preparation fixed; uncertain records do not by themselves determine a quantum state. The action, spatial adapter and time orientation are supplied. Physical clock calibration and agreement between the field cone and the separate event-count construction require their own arguments.
