...
One factor is owed, and chapter twenty-one supplied it. Entropy is heat divided by temperature. The temperature an accelerated observer reads is its acceleration divided by two pi, in the units where the speed of light, Planck's constant and Boltzmann's constant are all one. Dividing by that temperature multiplies the heat by two pi, taking the matter side to eight pi squared times the fourth power of the radius, divided by fifteen.
...
Set the sum of the two to zero, remembering that the area enters the total entropy divided by four times Newton's constant. Every appearance of the radius cancels. Both fifteens cancel. What is left is eight pi squared on one side against pi over Newton's constant on the other, and the ratio of those two is eight pi times Newton's constant, which is the coupling that stands in front of the stress in the field equation. Nobody fitted it. The eight pi that every textbook writes in front of Newton's constant without comment is the ratio of two elementary integrals over a small ball. What the machine-checked composition takes as given is the bundle behind those integrals: the balance of the nine-direction tomography, the Ward and Bianchi identities, the connectivity of the carrier, one universal coupling, a vacuum reference, and the identification of the constant in the area term with Newton's constant, so the equality of the coupling with eight pi times Newton's constant enters the theorem as a supplied field. The composition is a theorem on the finite carrier under those supplies rather than a derivation of a smooth Einstein manifold.

That gives one of the ten equations, in one observer's rest frame. The other nine come from the overlap rule. Chapter seventeen gave every observer a three-dimensional space of velocities, and observers passing through the same event at every velocity in an open range each hold the same relation in their own frame, which the overlap rule carries from one frame to the next. Written out in terms of the velocity, each relation is a polynomial of degree two. A polynomial that vanishes for every velocity in an open range has every coefficient equal to zero. Ten components, from a ball of observers who have to agree with each other.
...
Fields carried by the network keep to the same cone. Disturb a field in one slab of it and a detector in a separate slab responds. Before a light signal could have crossed the gap, the response shrinks to nothing as the network is refined.
...
Nine readings do it. Take the six rays that travel along one axis at a time, forward and backward along each of the three. Then take three more that share their motion equally among all three axes, each space component equal in size to one over the square root of three, with the signs plus plus plus, plus plus minus, and plus minus plus. The six axis rays never pick up the cross terms, the entries pairing one space direction with a different one, because a ray running along a single axis has no second direction to pair it with. The three tilted rays are there for those, and their three sign patterns read the three cross terms off one another. Nine light rays, nine numbers, one system of linear equations to invert. Its determinant is exactly eight thousand one hundred and ninety-two over twenty-seven, or two to the thirteenth over three cubed. A determinant that misses zero is a system that inverts, and this inversion is stable: small errors in the nine readings stay small in the nine numbers recovered from them.

Demand the balance of heat against area along those nine rays. Symmetry, conservation and the connectedness of the network carry it to every light ray there is, and what comes out is Einstein's equation with one constant term multiplying the metric. That term is the tenth number.
...
Add up the zero-point energies of the quantum fields of empty space, cut the sum off at the shortest length anybody takes seriously, and you get a vacuum energy density around a hundred and twenty orders of magnitude larger than the one the sky reports, the sky's value of the cosmological constant being about 1.1 times ten to the minus fifty-second per square meter. That is the worst prediction in the history of physics, and being wrong by that factor takes a kind of commitment a merely careless calculation cannot reach. The arithmetic in it is sound. What fails is the instrument. Every local calculation in physics is built out of light cones, light cones read zero on the vacuum's energy, and so no local calculation was ever going to fix that number.

What can fix it is a count that no small region contains: the total number of distinguishable records the horizon around an observer can hold. The smallness of the constant is then the largeness of that count. Chapter twenty-eight computes the count on two bookings of the reserve, selects neither, and compares both with the sky; the step that reads either count as the constant in the field equation is declared rather than derived.
...
That uncertainty is where the comparison stops being informative, and worth being plain about which side it sits on. Newton's constant is the least precisely known of the fundamental constants by a wide margin, measured to five figures where the electron's magnetic moment is known to twelve, because gravity is feeble and everything in the laboratory has mass. A derived value landing inside a window that wide passes a weak test. MICROSCOPE measured gravity's indifference to what falls at one part in a thousand million million, and the strength of the thing being indifferent is known to twenty-two parts per million, ten orders of magnitude worse, because a difference between two cylinders nulls out everything the two have in common and a magnitude has to be assembled out of a kilogram, a meter and a second.

## Order, number and curvature

Chapter seventeen constructed a flat causal and counting limit from a supplied population and read law. Its volume formula counts distinguished logical records under stated refinement conditions. Extending that construction to curved physical spacetime requires compatible source dynamics, event placement and counting measure.

Start with the order. The continuum theorem of Hawking, King and McCarthy and of Malament, met in chapter seven, recovers conformal geometry under its causal hypotheses. It fixes the light cones and leaves a local scale free: multiplying every length at a point by the same positive number leaves its cone unchanged. A compatible volume measure fixes that scale. Record counts can serve this purpose only when their calibrated limit is proved to equal that measure. Neither an arbitrary finite log nor the count of every primitive repair supplies it automatically.

A spatially flat expanding cosmological model illustrates the division. In conformal coordinates its metric is the flat metric multiplied by the square of a scale factor. The conformal change preserves the continuum causal order and multiplies four-dimensional volume density by the fourth power of that factor. If a record model is separately calibrated to this density, it can encode expansion in counts. Recovering a redshift from fourth-root count ratios additionally requires matched coordinate sampling and the corresponding source and observer histories. This is a conditional geometric correspondence; the local routing construction does not derive cosmological expansion or its record density.

Curvature changes the volume of a small diamond relative to a flat diamond of matched duration. A record count reproduces that correction only if the curved sampling and volume correspondence are controlled. Crossing reads have a related conditional formula: uniform spatial sampling with a spherical reach gives a leading flat-boundary count proportional to area, with coefficient one quarter of pi times density squared times reach to the fourth power. Applying this continuum calculation to actual seam histories requires the appropriate placement, sampling and read law. Added relay operations do not inherit it.

The geometric and thermodynamic arguments therefore specify what a common observer construction must realize: causal order, calibrated volume and boundary measures, an operational temperature, and the relevant small-ball entropy balance. Under those supplied conditions the continuum argument has Einstein's form. The finite classical routing and counting results do not establish all those conditions together.

This gives a conditional account of how order, measure and source balance determine geometry. Turning it into a physical reconstruction requires those inputs on one common history. Free fall as straight motion in curved geometry remains the guiding comparison, the same comparison the two walkers on the equator make concrete.
