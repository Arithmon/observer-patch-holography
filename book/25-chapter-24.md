...
A field can now be calculated on those same prepared addresses. Nearest neighbors exchange a scalar field through one specified action, which also defines its quantum states and detectors. In a finite calculation, a disturbance confined to one slab produces a response at a detector in a separate slab; the entire certified error stays smaller than that response throughout the observation window. The comparison is with a massive field in a cube with fixed walls. The addresses are stored separately from the changing field values. This construction still supplies its action and model time; matching that time to the recorded causal history is a further step.
