...
Chapter seventeen built flat spacetime out of records: an order from who read whom, a volume from how many. This chapter has been bending spacetime the whole way through, and the bending has to sit in the same two places.

Start with the order. The theorem of Hawking, King and McCarthy and of Malament, met in chapter seven, says the order hands over the light cones at every point, in curved spacetime as much as in flat, and with them every angle. It hands over nothing about size. That is a chart of the kind chapter seventeen's navigators bought, every crossing angle right and Greenland drawn the size of Africa. Of the ten numbers the metric holds at a point, the order fixes nine. The tenth is a local scale, and no light cone can see it, because multiplying every length at a point by the same number leaves the cone where it was. It is the blindness of the nine light rays over again, for the same reason. Counting supplies the tenth number. The number of records in a small region is its volume, which is the scale at that spot, so order and count together fix all ten numbers everywhere: the whole metric, curvature and all.

The expanding universe shows the division of labor cleanly. Space that expands evenly and is flat on the largest scales, which is what the sky reports, is flat spacetime with every length multiplied by one number that grows. Multiplying lengths moves no light cone, so expansion changes nothing in the order. Records reading their neighbors in an expanding universe produce exactly the partial order they produce in a static one, read for read. All of the expansion is in the counts. Follow one patch of the universe as it grows, and over the same number of rounds it holds more records the larger it has become, by the fourth power of the growth. The redshift of a distant galaxy, the stretch in its light that Edwin Hubble plotted against distance, is a ratio of counts: one plus the redshift is the fourth root of the records the patch holds now over the records it held when the light set out. The history of the expansion and the rate at which the universe writes records are one datum.

Curvature in general is more than one growing number, and the counts carry it too. A small diamond in curved spacetime holds a slightly different number of records than a flat diamond of the same duration, by an amount set by the curvature at its center. The geometry side of this chapter's derivation used two measurements on a small ball, its volume and the area of its boundary. The volume is a count of records. The area is a count of reads. In any one round, the number of reads crossing a flat surface is proportional to the surface's area, a quarter of pi times the square of the density of records times the fourth power of the reach, for each unit of area. The reads that cross a cut are chapter twenty-three's severed seams, written into the order.

Every ingredient of the derivation is then something the observers count. The light cones, and the horizons made of them, are the order. A volume is a number of records. An area is a number of crossing reads. The temperature is the flow of chapter twenty-one, which the state supplies. At the settled state the entropy of every small ball, contents plus boundary, sits at its maximum, and the counts obey Einstein's equation. Order plus number is spacetime with gravity in it, curved by exactly what is there.
