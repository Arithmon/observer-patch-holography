...
Chapter seven taught partial orders on a kitchen counter, before this world contained anything to order, and chapter nine supplied the missing ingredient. The events are commits: committed repairs, whose reads and writes are recorded, together with the records, readbacks and feedback that observers write down.
...
That is enough to settle every question about order that has an answer. Two repairs on seams that share an observer both read and write that observer's register. The later one worked from the register after the earlier one had written it, and its certified read carries the version it used, so there is a fact about which came first. Following another repair in an executor's list creates no such fact. Two repairs on seams with no observer in common have no direct fact of that kind. There is no register anywhere holding it, neither observer at either seam was in a position to write it down, and no clock or queue position could supply it afterwards, because supplying it would mean constructing the shared present chapter seven found nothing to build out of.

Two pairs, concretely. A repair on the seam between the first observer and the second, then a repair on the seam between the second and the third: the later move worked from a number the earlier move wrote, so any observer holding both records can see the dependency. The writer's mark on that number and the read certificate on the later move establish it, and the order between them belongs to the arrangement rather than to a convention. Against that, a repair on the seam between the first and the second and a repair on the seam between the fourth and the fifth touch four different observers, four different readings and no common register. Ask which of those two came first and neither record has anything to say about the other.

Count the pairs. Thirty seams make four hundred and thirty-five pairs of seams. A given seam has two ends, and at each end four other seams arrive, so eight seams share an observer with it and twenty-one do not. That gives one hundred and twenty pairs of seams whose repairs share a register and three hundred and fifteen that share nothing. Those counts belong to the wiring. The order in any one run comes from the versions its commits actually read, and a chain of repairs through other seams can order two repairs that share nothing.

That is a great deal less order than a line would carry. A line puts everything in single file and answers all four hundred and thirty-five pairwise questions before a single repair has run. The observers instead produce a web after the fact. Every strand is a certified dependency of a read on a write, and commits with no chain between them stay unordered. The wiring says where dependencies are possible. The records say which ones occurred.

The everyday picture flattens that web into a timeline. The difference shows up in what you are allowed to ask. "Which of these two happened first" has an answer for some pairs and none for others. Which pairs those are is fixed by the records themselves rather than by any observer's ignorance.
...
That is what a repair **commit** does: it leaves the total alone, satisfies the seam, and discards which disagreement it resolved. The acceptance test of chapter nine refuses any move that makes the fit worse anywhere, which rules out the move that would put 6 and 0 back. The ruling would be academic anyway, because nothing at the seam knows which of the seven to restore.

Those two facts fail differently. The acceptance test is a rule about which moves are available, and a rule can in principle be relaxed. The overwriting is arithmetic. Seven into one is a fact about averaging whole numbers, and no amount of relaxing anything recovers the input from the output.

A write can be made undoable, and the method is old: keep a copy. Save 6 and 0 into a blank register before averaging them and the seven cases stop being interchangeable, because one of them is written down somewhere else. Copying onto a blank register erases nothing. The bill arrives when memory runs out and a register has to be cleared for reuse, and clearing it is another write that discards what it held. Every route out of the collapse arrives at another commit.
...
Three cases, by hand. A fair coin has two equally likely outcomes, and the answer is exactly 1 bit. A coin biased nine to one comes out at 0.469 bits, and that one is the only one of the three with any arithmetic in it. Nine times in ten the answer is heads, worth 0.152 bits of surprise, contributing 0.137. One time in ten it is tails, worth 3.322 bits, contributing 0.332. The two add to 0.469, so a coin that lopsided carries less than half a bit. A fair eight-sided die gives 3 bits, one for each halving of eight. Give the seven arrangements at the seam equal odds and they come to log base two of seven, which is 2.807 bits. A commit on that seam removes 2.807 bits from what the seam can say about what it used to hold.
...
A repair step is a process, and it preserves the reference. So the relative entropy of any description against the reference never climbs, step after step. That is the second law, in the arrangement, with no appeal to counting microstates and no claim about how anything started.
...
And the average of the entropy produced, over all transitions weighted by how likely they are, equals the fall in relative entropy exactly. The mean of the quantity that fluctuates is the descent itself, on the nose, with no correction term. The second law then follows in one step. The exponential curve bends upward, so averaging it overshoots the exponential of the average, a fact called Jensen's inequality. The average was exactly one, and the only way both can hold is for the mean entropy production to be at least zero. Nothing anywhere in the argument is approximated.

One more relation drops out of the same bookkeeping. Take two quantities you can measure about the arrangement, evaluate the first before a step and the second after, and average over the settled state of a balanced repair. Swap which one you evaluate first. The two averages are equal. Lars Onsager published that symmetry in *Physical Review* on 15 February 1931, worked out its consequences for coupled processes such as the way a temperature difference across a metal junction drives an electric current and a current drives heat, and collected the Nobel Prize in Chemistry for it in 1968. Transport coefficients come in pairs and the pairs are equal. Here it is a two-line consequence of the same relation that gave the arrow.
...
The machine ran a hundred thousand attempts. The expected number of successes at that rate is about a tenth of one success, and the number obtained was zero, which is what nine and a half in ten million looks like when you are only allowed whole ones. Computing power buys more attempts and leaves the odds on each one exactly where they were. The bits were discarded by the commits that made the history, so there is nothing to copy and nothing to compute.

The past is the set of commits that have been made, together with the certified reads that tie them to one another. Its content is fixed because rewriting it would need information the writes destroyed. The future is the disagreements that have no commit yet. They are not being kept from anybody: their content does not exist in any register in the world, because the repairs that would fix it have not been made.

Chapter six left one unit of disagreement sitting in a world of twelve. One flipped record, thirty seams, nineteen loops, and a leftover of exactly one that no allowed repair removes. Repair in a different order and it sits somewhere else. Every move that put it where it sits was a commit, each one discarding the arrangement it came from. A defect that survives every repair the world can perform is a thing with a history. The history is the chain of writes that carried it, in an order that any observer holding two of those records can check.

Every strand in that web was laid by one commit reading what another commit wrote. Chapter nine counted how fast that can spread: one seam per event, no faster, so a commit can only depend on commits within a certain number of seams of it. The edge of the web advances at a fixed number of seams per event. Turn that count into meters and seconds and it is a speed. Every laboratory that has ever measured that speed has come back with the same number, whatever the laboratory was doing at the time.
