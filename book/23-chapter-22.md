...
The everyday version of the same question sits on your desk. A drive with no moving parts gets warm while it is being written to. Most of that warmth is ordinary electrical loss, and engineers shave it down every year. Underneath it sits a floor that no engineering removes, in any machine anybody has built.
...
The books balance only if the bit costs at least what the bit earns. Rolf Landauer, at IBM, worked out in 1961 which operation carries the charge, in a paper called "Irreversibility and heat generation in the computing process". Looking can be done for free. Forgetting cannot. An operation that takes two distinguishable states of a memory to one state has thrown away the distinction. The distinction has to leave the device, because the only exit a memory has is heat. The floor he computed for that heat depends on nothing but the temperature outside the device.

$$E \geq k_B T \ln 2$$

E is the energy released as heat when one bit is erased, T is the temperature of the surroundings the erasing device sits in, and the constant converts a temperature in kelvin into an energy in joules. The logarithm of two is chapter sixteen's one question, the one you ask to tell two possibilities apart.
...
measurement it has to clear space. Clearing the space costs at least the temperature times the logarithm of two per bit, which is everything the molecule paid it. The demon was never
...
The first law gets taught as bookkeeping, in two terms. The energy of a thing changes when heat flows into it and when work is done on it, and heat and work are the two channels. Work moves the energies of the available arrangements while the odds over them stay put. A piston does that. Heat changes the odds while the energies stay put. Contact with a flame does that. Do both at once in one finite jump and a third term appears, the product of the two changes, which vanishes when you change one thing at a time. Over a smooth change that product shrinks faster than either change does, which is why the textbook's two terms are exact for a process that runs continuously.

The second law arrived in chapter sixteen, in a laboratory in Canberra with a latex bead in water. Take the odds a settled arrangement assigns, call them the reference, and ask how far your own description sits from it. Under any process that leaves the reference alone, that distance never climbs.
...
The full map settles everything unsettled in a single step. Local repair, working seam by seam, takes longer and keeps the same public record the whole way. The smallest exact case has eight states: one recorded bit held fixed, and two more bits redrawn from their odds given that record. The correlations between the two decay toward equilibrium, and the recorded bit never moves. Memory and relaxation run side by side in one machine.

The integer averaging rule has another way to carry memory. Once every port holds one of two neighboring whole-number loads, fair repair can keep swapping those loads without changing their total. Let each patch record its load while these exchanges continue. On a fixed connected network, with the stated random timing, slowly relaxing patterns leave stronger correlations in a long record than quickly relaxing ones. Their strength can be calculated from the repair rule and the prepared total. A Gaussian history limit follows without injecting Gaussian noise at the ports. The length of the record and the scale at which it is read matter; this finite result supplies no measured cosmological amplitude, physical clock or expansion history.

There is a precise limitation to the geometric interpretation. In this repair model, exchanging loads leaves the assigned cell shapes and volumes fixed. A record of those volumes therefore contains no changing geometric disturbance. Choosing a different load readout does not settle the question: two local readouts can have the same variance and different correlations between separated ports. A cosmological interpretation needs a rule connecting the changing records to changing geometry.
...
Chapter sixteen's detailed fluctuation relation leans on one extra condition: measured against the reference, the weight a step carries from one state to a second matches the weight it carries back. The condition has a name, **detailed balance**. The integral identity of that chapter does without it.
...
The bit had to go somewhere. It leaves the two observers as heat, at least the temperature times the logarithm of two of it, at whatever temperature the pair's own state sets, which is the multiplier in its own exponential. Copying the old readings somewhere first only postpones the bill, for Bennett's reason: the copy sits in a finite memory, and clearing that memory costs the same. Chapter sixteen showed that a commit discards which of seven arrangements the seam used to hold, and priced it at 2.807 bits of the world's ability to say what it used to hold. The same commit has an energy attached: 2.807 bits of erasure at three hundred kelvin costs 8.06 divided by ten to the twenty-first joules, expelled as heat, per seam, per commit, forever, everywhere in a universe that is making a public record out of private disagreements. The joule figure is the price a physical memory pays for that reset against a reservoir at three hundred kelvin under Landauer's conditions; a commit in the model supplies neither the reservoir nor its temperature, so the price is what a laboratory realization of a commit would pay.
...
Every record ever written has been paid for in heat, at the temperature of whatever wrote it. The heat leaves through the edge of the region holding the record, putting a bill on the edge rather than on the volume inside it. Ask how many records a region of space can hold, and the answer comes back in square meters, on the horizon-record branch that calibrates the count against physical entropy and area; chapter twenty-eight sets out that branch and its two bookings of the reserve.
