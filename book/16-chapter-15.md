...
The rank-three slow band gives an abstract continuous three-dimensional Euclidean carrier under the stated repair rule and response metric. Adjoining a real axis gives a four-dimensional Lorentz carrier with a two-sphere of null directions. Chapter seven supplies the authenticated event order. The physical reconstruction must join these results: the events must fill the carrier with compatible causal relations, clocks and volume scaling. A long sequence of records need not fill a volume, even when each observer has three local directions. An exact finite order embedding is one route. A controlled approximation whose causal error vanishes under refinement is another; the growing-direction lattice construction in the spacetime paper demonstrates that mechanism for a supplied law. Neither the local band nor this control selects the physical event family. The pages below derive the local three-dimensional carrier.
