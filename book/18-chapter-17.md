...
The cone so far came off the sky: a sphere of directions, the maps that keep its angles, a determinant that vanishes. Chapter seven's order came from somewhere else entirely, records that read what other records wrote. If the two are one object, records reading records should fill out this cone and no other. They do, and watching how they do it is the whole route from bookkeeping to spacetime.

Start with chapter fifteen's six counters. Every record carries six whole numbers, every tally lands at a point of the three-dimensional band, and the landing places pack the band more finely the more records there are. Take a population of records dense enough that no point of some region lies further than a small gap from one of them. Then let them read. In each round every record reads every record that lay within a fixed reach of it in the round before, itself included, and writes a new record. Reading your own previous record is what waiting looks like. Nothing else goes in: no clock, no grid, no list saying who follows whom. The partial order is whatever the reads make it.
...
For the number, pick two events, one in the other's future, and take everything after the first and before the second. The shape is two cones joined at the rim, a **causal diamond**. As the population is refined, the number of records inside any diamond, times one fixed weight per record, approaches the diamond's four-dimensional volume. It grows as the fourth power of the time between the tips. Twice the time, sixteen times the records.

Counting also says how many dimensions there are, with no ruler anywhere. Take every pair of events inside a diamond and ask what fraction of the pairs are ordered, one inside the other's cone. With one direction of space the fraction is a half. With two it is eight thirty-fifths, a little under a quarter. With three it is one tenth. Every extra direction gives two events more ways to miss each other. Jan Myrheim worked the fractions out in 1978 and David Meyer made a dimension meter of them in 1988. For the records, the fraction inside every diamond approaches one tenth: three directions of space and one of time.

The observers ran this with one twelve-port carrier standing in for each record, holding its six whole numbers as loads on its six pairs of opposite ports. Each carrier read its own position off its own loads, decided for itself which others lay within reach, and then read them, round after round, every read checked against the writer's mark. With 39,304 carriers reading for six rounds, the diamond at the center holds 32,265 events. Of the 520,498,980 pairs among them, 47,881,819 are ordered, 9.2 percent. The order generated by the certified reads alone, with nobody handed a list of parents, is exactly the order the reach predicts. A nudge given to the center record at the start turned up everywhere in its future cone and nowhere else. Run the same rule on 166,375 positions for eight rounds and the fraction comes to 9.3 percent, and the dimension it implies is 4.09. Confine the positions to a plane and the meter reads 3.02; to a line, 2.01.

Divide the count of records in one diamond by the count in another and take the fourth root, and the answer approaches the ratio of the durations a clock would show riding from tip to tip in each. The root is a fourth root because there are four dimensions, and no other clock is available: any clock that reads only how many records lie between two events, and adds up correctly along a straight path, is the fourth root of the count, up to a choice of unit. No tick went into it. The second diamond sets the unit, and where the clock starts is anybody's choice.

String small diamonds along a path and add up their clocks. Against a path at rest, a moving path passes through thinner diamonds holding fewer records, and collects less time, by exactly the factor the light clock on the train lost against the platform. That is the part of Hafele and Keating's result that came from speed. The part that came from height above the ground is gravity, and chapter twenty-four takes it up.

So records reading their neighbors, round after round, are order plus number, and order plus number is spacetime with four dimensions: three directions from the slow band, one from the way reads run, a light cone from the reach, and volume and duration from counting. The speed of light is one reach per round.
...
The six motions shuffle the sky as hard as you like and leave the cone exactly where it was, because the cone is where a determinant vanishes and the six motions preserve determinants. So every observer sorts every pair of events into the same three classes: influenceable, not influenceable, and the boundary. Drawn the way the last section drew them, the records' own order and the cone's order agree in both directions. Two events either have an order that everybody can read or have none at all, which is the web of chapter sixteen with its strands drawn in. The boundary between the two situations is a property of the sphere rather than a property of light.
