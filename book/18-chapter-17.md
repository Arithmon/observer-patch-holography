...
Set the determinant to zero and you have the light cone. A two-by-two block with determinant zero has one of its rows a multiple of the other, so it carries one independent row rather than two. Chapter fifteen called that count the rank. Light rays are rank one, and every other nonzero separation is rank two.
...
Pick a direction into the future with determinant one and call it an observer at rest. Ask which vectors are perpendicular to it, using the interval to decide perpendicular rather than ordinary distance. The answer is a space of three dimensions, the observer's rest space. On that space the interval with its sign reversed is positive for everything except zero, ordinary Euclidean distance in three dimensions. Every observer gets one. It is the same three directions chapter fifteen read off the slow band of the carrier.
...
The cone so far came off the sky: a sphere of directions, the maps that keep its angles, a determinant that vanishes. Chapter seven's order came from somewhere else entirely, records that read what other records wrote. A specified rule for those reads gives a mathematical route between the two. Its assumptions matter: the record population, the permitted reads and the way a round is assigned a duration are supplied. Identifying the resulting cone with physical causality is a separate step.

Start with chapter fifteen's six counters and prepare the specified population of records. Their whole-number tallies land at points in the three-dimensional band, with no point of the chosen region further than a small gap from one of them. Protect those addresses while other values change. In each round every record reads every record that lay within a fixed reach of it in the preceding round, itself included, and writes its next value. Reading your own preceding record is what waiting looks like. The reads determine a partial order. The complete neighborhood rule, protected storage and round structure are part of the construction, rather than consequences of pair averaging alone.

Ask how far one event's influence can have got after five rounds. Two plain facts settle it. A read covers at most one reach, so nothing has got further than five reaches. And anything nearer than five times the reach, less twice the gap, can be got to. Draw a straight line to it, mark five evenly spaced waypoints along the line, and in each round step to the record nearest the next waypoint. Each step is a fifth of the line plus at most a gap at either end, and that fits inside a reach. So the influence of every event fills a region caught between two cones, and the only difference between them is twice the gap measured against the reach. Refine the population so that the gap shrinks faster than the reach, and the two cones close onto one. Its speed is one reach per round.

On a grid where every move runs along one of three axes, influence would spread in no round cone at all: a point two steps east and two steps north lies within three steps as the crow flies and takes four moves to reach, and the region of influence comes out as an octahedron, two pyramids base to base. Here the reach is a ball, set by distance alone, and the sixty rotations of chapter fourteen carry it onto itself. A theorem says the round cone is the only one possible. Suppose possible influences form a cone, so that possible displacements combine into possible displacements and no displacement is possible together with its reverse. Suppose the sixty rotations leave the cone alone, and so does the sky's crowding along one single direction. Then the cone is exactly the one the sky's six motions preserve, the light cone, opening toward the future or toward the past. The rotations alone would allow a cone of any width. One crowding fixes the width, and the rotations carry that one crowding round to all three directions. The records pick the future, because height goes up along every read.

A finite log fits the cone with no limit taken, once each event is drawn at its position with its height times the length of a round as its time. Every read then stays inside the future cone, since a read covers at most one reach in one round. Where the positions also keep events with no chain between them outside each other's cones, order and cone agree exactly, pair by pair, and every stretch of the order is the matching stretch of the cone.

For the number, pick two events, one in the other's future, and take everything after the first and before the second. The shape is two cones joined at the rim, a **causal diamond**. In the specified refining family, count one completed record per site and round and give it the family's prescribed weight. For timelike tips approaching fixed positions, with the whole limiting diamond inside the chosen region, that weighted count approaches the diamond's coordinate volume. The volume grows as the fourth power of the model duration between the tips. Twice the duration, sixteen times the limiting weighted count. A diamond cut by the boundary requires a different comparison.

Counting also gives a dimension test. Take every pair of events inside a flat spacetime diamond and ask what fraction are ordered, one inside the other's cone. With uniform volume sampling and one direction of space the fraction is a half. With two it is eight thirty-fifths, a little under a quarter. With three it is one tenth. Every extra direction gives two events more ways to miss each other. Jan Myrheim worked the fractions out in 1978 and David Meyer made a dimension meter of them in 1988. The specified record family approaches one tenth for the contained timelike diamonds covered by its limit theorem. A finite or clipped sample need not give that answer.

Software executions implement this declared read rule with records represented on twelve-port carriers. The program reconstructs each position from its prepared six counters, computes the prescribed neighbors, and checks every read against its writer's mark. With 39,304 records and six rounds, the outer central interval holds 32,265 events. Of its 520,498,980 pairs, 47,881,819 are ordered, about 9.2 percent. A starting disturbance reaches exactly the records reachable through the read graph. On 166,375 positions and eight rounds, the outer interval gives about 9.3 percent and a fitted dimension of 4.09. Both intervals are clipped by the coordinate boundary, so these figures are finite diagnostics rather than tests of the contained-diamond limit.

Carrying those reads through local seams is a further construction. With supplied export, reset, capture and scheduling rules, complete routed executions serve 2,197 and 9,261 records. Each run uses about nineteen million and 293 million primitive events respectively. A separate checker reconstructs every value and consumed writer. The completed logical records have the intended order; the intervening relay and housekeeping operations are also counted as costs. They do not acquire the logical records' volume weight. Giving every added operation that same weight changes the count, and cannot preserve the stated volume limit.

Storage can be reused once the prescribed program has finished every read of an old value. Two alternating payload stores and temporary relay space suffice under that lifetime rule. This bounds scalar slots, not the digits needed for exact values, controller memory or the separately retained audit history. A request for a retired version falls outside the interface.

Pair averaging can also implement a reusable accumulator after one preparation. Values carried as differences between two nonnegative loads remain readable as their scales change; clearing the accumulator retires its old answer. A receiver publishes only when its samples and supplied error bounds admit one answer. If several answers fit, it abstains. These constructions require prepared inputs, chosen requests, a controller and a readout instrument. The accumulator alone does not preserve every completed answer.

With two protected banks, pair averaging can also store computed answers for later layers and reuse records after their last requested read. One bank stays readable while the other receives the next layer. Complete finite examples check this process, including signed inputs and a change to the central input. A single error budget covers preparation, transport, cleanup and publication. The bank placement, request lists and controller are supplied. The construction neither selects those inputs from the observer axioms nor supplies physical isolation, a clock or attainable precision.

There is also a way to recover initial values while changing the storage that holds them. Imagine connected cups whose contents can be averaged in pairs, with one cup sampled exactly. First measure that cup. Bring in an unmeasured neighbor by averaging along a path of cups whose contents are known, ending at the sampled receiver. The receiver's measurement and the known relay values let us undo the averages arithmetically and recover the neighbor's former value. Repeating this through the connected region recovers every starting value from samples at the same receiver. The paths come from the connections; the method assumes exact samples, retained measurements and arithmetic. It does not preserve the original contents or their separate versions.

A specified publication requirement can also determine a distribution over read sequences. Fix the records to recover, the allowed moves, a deadline and a positive reference probability for each complete sequence. Suppose at least one sequence meets the requirement by that deadline. The distribution adding the least information to the reference, while satisfying the requirement, is the reference restricted to successful sequences and normalized. Summing the reference weights of successful continuations from each step gives a controller that samples this law as it goes. Failed physical runs need not be executed and discarded. The requirement and reference remain inputs, and different deadlines can give incompatible distributions. This finite construction selects a schedule within its declared interface. It supplies neither the geometric reach used above nor a physical clock or an attainable measurement precision.

The deadline can be removed by calculating the probability of eventual recovery from each intermediate state. Weighting the next move by that remaining probability gives distributions that agree when a long sequence is shortened.

On a chain of cups there is a stronger result: place independent unknown amounts at cups separated by calibrated empty cups, and every finite sequence of neighboring averages retains enough independent information in the current contents to reconstruct the unknowns. If every move keeps a uniformly positive chance of occurring, retained exact samples at one receiver eventually recover them all with probability one. No preservation test is needed for this prepared chain, although the required precision can grow without bound. This recovers the whole preparation; it does not choose which records a geometric neighborhood should require.

Several shrinking reaches, and both golden and uniform populations, pass the same limiting cone and counting tests while having different finite reads. The exact square-root formula for that reach is unnecessary for these continuum limits. It balances two errors in the existing estimate: smaller steps improve time resolution but accumulate more spatial approximation error. This explains a useful regulator, not a fundamental physical length.

Removing every spatial restriction would allow any distance to be crossed in one shrinking time step; keeping only the nearest axial grid neighbors instead gives a different, diamond-shaped spatial reach. The construction requires an appropriate source-derived local propagation law; it does not require precisely the chosen square-root regulator.

There is a precise limit to what maximum randomness can contribute here. For the finite quantum state problem itself, with positive observer weights and reference states that assign positive weight to every direction, a failure that is possible in any allowed state keeps positive probability in the selected state, provided it is measured by an effect in a scored observer algebra or by a positive restriction from one. This holds even when the local states do not commute and no single global state extends them. A quantitative bound follows from any allowed failure witness. Exact success therefore requires the source constraints to exclude failure; an entropy optimizer cannot supply that exclusion. The bound can shrink as the resolution grows, so this does not rule out success in a limit.

A separate two-time example makes the distinction concrete: preserving the same maximally random state permits both perfect retention of a classical bit and complete erasure of it. Choosing a state has not chosen the process that carries its records.

The registered all-port simulator capture can also be examined without assigning it a desired read geometry. Its scalar ledger updates pair port slots in disjoint pairs. Once a pair has been averaged, further visits do nothing; no sequence carries a value through a second pair. A receiver given its initial values and all later local samples can recover its visited partners, but distinct positive, normalized preparations produce identical local payloads while disagreeing elsewhere. The local complex evolution needs more information: equal intensities with different phases can give different subsequent intensities. These results identify what this particular executable transmits and what additional state a recurrent instrument would need. They concern numeric records, not the global archival hashes also carried by the software, and do not establish that the executable implements the complete axioms.

For the specified family of logical records, divide the counts in two suitable diamonds and take the fourth root; the answer approaches the ratio of their model durations. The exponent comes from four-dimensional volume. A count-only duration that obeys the specified continuity, scaling and straight-path addition rules has this form, up to a unit. A reference diamond sets that unit. Relating the result to a laboratory clock requires an operational comparison; neither the supplied duration of a round nor the number of relay operations provides that calibration.

String sufficiently resolved, contained diamonds along a path and add their count durations. Under the model's refinement conditions this approaches proper time. A moving path collects less than a path at rest by the familiar relativistic factor. The mathematical comparison has the form used for the speed contribution in Hafele and Keating's experiment. It does not itself establish that the simulated records are their clocks. The contribution from height above the ground involves gravity, which chapter twenty-four takes up.

The supplied record family therefore has a four-dimensional causal and counting limit: three position coordinates, an order generated by reads, and coordinate volume and relative duration recovered from suitable counts. Its cone speed is one reach per declared round. Selecting the population and read law from the observer dynamics, and identifying that model speed and clock with physical measurements, are distinct requirements.
...
The six motions shuffle the sky as hard as you like and leave the cone exactly where it was, because the cone is where a determinant vanishes and the six motions preserve determinants. They therefore agree on the cone's three classes: timelike, spacelike and null separation. Where a finite event placement satisfies the two-way order condition described above, its record order agrees with that classification. The refining read model supplies the corresponding limiting cone under its own hypotheses. Attaching either construction to observed light requires the physical identification.

The cone speed becomes a physical speed only after lengths and durations have physical units. In the read model it converts one prescribed reach into one logical round. Local transport can use many primitive operations within that round, so counting seam operations does not provide the conversion by itself. The angle-preserving sky maps explain why a common Lorentz cone has a common invariant speed; they do not calibrate rods or clocks.
