...
A finite log need not determine the whole network that produced it. In a declared network of 1,280 twelve-port carriers, the recorded repairs and reads use 86 of its 7,650 connections. One reconnection of two unused pairs preserves every recorded operation, even though the number of triangles in the complete network changes. The two networks cannot be renamed into one another. Their local states, ports, readback and protected records support the same observed history. Recovering the whole wiring therefore needs more observations or a justified restriction on which networks are allowed. The equality demanded of a self-reading world is a condition to establish; replaying one finite history does not establish it.
...
There is a dividend in that. A description obliged to specify the structure it came out of cannot leave a number loose, because a loose number is a second structure answering to the same description, and the fiber has to hold one member. Chapter twenty-eight is where that gets cashed. The grain and the fine-structure constant are written in terms of each other, and the grain also fixes the record capacity, on either booking of the reserve, and through it the cosmological constant. On each declared map the loop admits one solution, and the choice of map is the one step the axioms leave open. A quantity that every other physics has to measure and enter by hand is computed instead. Constants are what a world with no outside is not free to leave open.
...
These results supply two ingredients for the proposed loop. They do not show that a finite record determines the architecture that produced it. Observers can reconstruct and replay the recorded program; identifying the full hardware requires additional observations or restrictions on the allowed structures. Equality between the constructed realization and the inhabited world is the closure hypothesis.
