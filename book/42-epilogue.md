...
15. Why does the position carrier have three directions? On the declared operational-cost branch, the slowest-fading response block has rank three, and its metric completes it to continuous abstract three-space. Conservative record populations with complete local reads and a model clock have a proved flat spacetime limit: their order approaches the light cones, and normalized event counts approach four-dimensional volume. Selecting that population and law physically, calibrating the clock, and identifying the same geometry in matter remain separate requirements.
