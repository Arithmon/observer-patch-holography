...
15. Why does the position carrier have three directions? On the declared operational-cost branch, only the slowest-fading block survives enough rounds to carry the candidate position readback, and its rank is three. The response metric completes it to continuous abstract three-space. Joined to an independent real axis, it gives a one-plus-three ambient Lorentzian carrier. Authenticated parent edges compute exact longest-chain source height, and an auxiliary enumeration places every finite log injectively and forward-causally in the carrier. Neither construction derives intrinsic poset dimension, physical coordinates or a clock; physical spacetime requires a source-selected refinement with controlled causal agreement, independent dimension and manifoldlikeness tests, and calibrated clocks and volumes.
