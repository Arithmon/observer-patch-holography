...
What produces a wave is a second quantity that the first one feeds. A pendulum has two: displacement sets the rate at which velocity changes, and velocity sets the rate at which displacement changes. The pair circles instead of settling. On the carrier the two quantities are the mismatch and the current carrying it. Repair conserves the sum it touches, so mismatch that leaves a port is carried off rather than deleted, and the seam operator supplies the stiffness that couples the pair. The mismatch sets how fast the current changes, and the current sets how fast the mismatch changes. That pair does not settle. It travels.
...
The stiffness is computed from the complete thirty-seam operator. At long wavelengths it approaches the square of the wavenumber. Its first correction subtracts a twentieth of the fourth power times the square of the seam spacing, and an exact bound holds across the full operator: the difference from the square of the wavenumber never exceeds that correction in magnitude. At zero wavenumber the stiffness is exactly zero, so the frequency there is exactly zero, and a disturbance of unlimited wavelength costs nothing whatever. That exact zero, with the stiffness rising as the square of the wavenumber above it, is why the long waves all travel at one speed. The speed is the ratio of frequency to wavenumber, which chapter nine fixed by counting one seam per event.

This controls the whole evolving field. Lay the seam-shaped operator across three-dimensional space and shrink its spacing. The paired fields converge to Maxwell's equations, with an error bound proportional to the square of the spacing for sufficiently smooth initial fields. Every field of finite total energy converges, even without that smoothness. A current can be included, and conservation of its charge preserves Gauss's law.
...
A finite software observer can read its own field without keeping a spare copy. It remembers a port's value, averages that port with an adjacent value, and reads the average. Twice the average minus the remembered value recovers the hidden value. Writing the recovered pair back lets the next measurement start from the same state. Applied to forty-two potential coordinates, this procedure produces the records that drive a sourced Maxwell update, and the fields reconstructed from those records satisfy the same finite action, including the charged paths.

The recorded potentials also define fields inside a solid built by joining the carrier's twenty triangular faces to its center, which gives the surface description an explicit volume. A geometric interpolation preserves magnetic flux closure and Faraday induction. Adding up the field energy throughout that volume gives a different action from counting its stored coordinates, and interpolation between time steps adds a separate correction. Both differences are calculated on the recorded history.

The volume can also carry its own evolution. Its geometric action gives equations for every edge, including the edges running through the interior. A field that starts out satisfying Gauss's law keeps satisfying it, and an exact bound controls its field energy. A software observer reads and restores fifty-five potential coordinates and uses the decoded records to advance the field, so the records carry a complete history of the volume that keeps its action stationary.

The same geometric action has thirty independent radiative modes. Quantized by the usual rule, they become thirty quantum oscillators. Their occupation numbers form an infinite Hilbert space, and a self-adjoint Hamiltonian evolves its states unitarily. That is the free radiative sector.

Dynamical matter enters as a charged scalar field. Its interpolation changes with the electromagnetic potential as gauge symmetry requires. Both fields follow one stationary action, and positive conserved energy ensures global classical solutions of the finite equations. A short numerical evolution starts with opposite charges near the center and the boundary. Five software patches record, probe and restore the symmetry-reduced state, and each advance reads those decoded records. Independent reconstruction checks every finite field equation. Exact interval arithmetic encloses the same solution throughout two units of time, and replaying the observer records reproduces all five positions and five velocities at its eighty-one decoded checkpoints to within $1.0001\times10^{-10}$ of that solution.

A real scalar sector gives a controlled continuum of evolving matter. Its electromagnetic fields vanish and its quartic self-interaction stays. Project a smooth continuum solution onto the finite mesh and subdivide the solid without making the tetrahedra progressively flatter, and the motion approaches the continuum nonlinear wave equation. The error in the field and its velocity decreases with the cell size over a fixed time interval. The same convergence rate holds for complex matter in a uniform magnetic field, with initial data projected using its magnetic energy.

The changing configuration also keeps time. Its kinetic metric and conserved energy assign a duration to every stretch of motion with positive kinetic energy, whatever labels were used to order the samples. Applied to decoded records, this approximates the action's elapsed time without reading a single timestamp.

The full interacting action has a Hilbert-space description as well. Removing redundant gauge descriptions gives a complete positive kinetic metric. Its volume measure and its Laplace--Beltrami operator, the curved-space version of the ordinary Laplacian, fix a Hamiltonian with a unique self-adjoint extension, and that Hamiltonian evolves neutral states unitarily. An explicit normalized initial state gives exactly calculated matter observables.

The classical charged state also provides position and momentum values for preparing quantum states, once the redundant gauge coordinates are removed. Gaussian packets centered on those values keep all fifty-six configuration directions. Averaging over the remaining common phase produces neutral states with an exact positive normalization formula, and their scalar radius can be calculated.

A trial history on the full fifty-six-dimensional configuration space changes the centered state's phase and leaves its configuration probabilities fixed. It stays within one tenth of the exact quantum evolution in Hilbert norm for times up to $2^{-55}$ in the action's units.

A separate comparison theorem states what a useful effective quantum description must control. Preparation error, the difference between the full Hamiltonians, detector mismatch and probability outside the chosen energy window together bound the error in measured probabilities. Coupling to omitted states matters even when the retained matrix entries agree exactly.
...
The single direction is the one in which all twelve ports move together, so no rotation of the carrier touches it, which puts it in the center. Chapter nineteen's center is what commutes with everything. A mode that commutes with everything carries no charge under anything at all, including itself. Its curvature has no term quadratic in the field, and its second-order response is Maxwell's action and Maxwell's energy density. Removing the gauge redundancy and the field constraints takes two of its four components away and leaves two transverse ones. That is the photon. Its two polarizations are the rank of a projector.
