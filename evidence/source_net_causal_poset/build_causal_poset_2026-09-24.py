#!/usr/bin/env python3
"""Rebuild the expected OPH causal poset at one level from its definitions (generator of the 2026-09-24 receipts).

The 2026-09-09 generation keeps its own frozen generator, ``build_causal_poset.py``, because other
packages pin that file by hash; this copy adds the q = 89 level, the two receipt generations and
the dimension-aware count-clock exponent of the current receipts.
...
Usage (from the package directory or anywhere):
    ...
    python3 build_causal_poset.py --generation 2026-09-09 --q 5 8   # the retained first generation
...
GENERATIONS = {
    # label: (family receipt, carrier receipt); the newest generation is the default
    "2026-09-09": ("source_net_causal_limit_receipt.json", "carrier_source_net_receipt.json"),
    "2026-09-24": ("source_net_causal_limit_receipt_2026-09-24.json", "carrier_source_net_receipt_2026-09-24.json"),
}
CURRENT_GENERATION = "2026-09-24"
SOURCE_NET_RECEIPT = HERE / GENERATIONS[CURRENT_GENERATION][0]
CARRIER_RECEIPT = HERE / GENERATIONS[CURRENT_GENERATION][1]
...
def build_level(q: int, dim: int = 3, pairs: bool | None = None, reads: bool | None = None,
    ...
    # The volume clock in dim+1 dimensions is the (dim+1)-th root of the count ratio: the fourth
    # root for the source net, the cube and square roots for the controls. Receipts of the
    # 2026-09-09 generation carry no exponent field and used the fourth root for every family.
    if "clock_exponent" in clock:
        root = 1.0 / (dim + 1)
        R.check("count clock exponent", f"1/{dim + 1}", clock["clock_exponent"])
    else:
        root = 0.25
    R.check("count clock (N_K/N_K')^(1/(dim+1))", rounded((NI / NJ) ** root), clock["count_clock"])
    R.check("count clock model-time ratio K/K'", rounded(K / Kp), clock["model_time_ratio"])
    R.check("count clock relative deviation", rounded((NI / NJ) ** root / (K / Kp) - 1.0), clock["relative_deviation"])
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--generation", default=CURRENT_GENERATION, choices=sorted(GENERATIONS),
                        help="which receipt generation to rebuild against (default: the newest)")
    args = parser.parse_args(argv)
    global SOURCE_NET_RECEIPT, CARRIER_RECEIPT
    SOURCE_NET_RECEIPT = HERE / GENERATIONS[args.generation][0]
    CARRIER_RECEIPT = HERE / GENERATIONS[args.generation][1]
