...
EXACT_PROFILE_LIMIT = 40000  # every diamond and sprinkling of the four levels takes the exact between-count matrix (the cluster sampler of the first drafts was biased by 6.6 percent at q = 34)
...
def exact_direction_count(dm: np.ndarray, db: np.ndarray) -> int:
    """Number of distinct directions among displacement vectors with components m + b*phi, exactly in Q(phi).

    Two vectors have the same direction iff one is a positive multiple of the other, and the multiplier of
    two vectors with components in Q(phi) lies in Q(phi); so every vector is normalised by its first nonzero
    component, (a + b phi) / (c + d phi) = ((ac - bd + ad) + (bc - ad) phi) / (c^2 + cd - d^2), and the sign of
    that component as a real number is kept.  No float enters, so the count is the same on every platform.
    """
    keys = set()
    for row_m, row_b in zip(dm.tolist(), db.tolist()):
        k = next(i for i, (m, b) in enumerate(zip(row_m, row_b)) if (m, b) != (0, 0))
        c, d = row_m[k], row_b[k]
        den = c * c + c * d - d * d
        comps = tuple((Fraction(m * c - b * d + m * d, den), Fraction(b * c - m * d, den)) for m, b in zip(row_m, row_b))
        keys.add((k, phi_sign((c, d)), comps))
    return len(keys)
...
def link_directions(fam: Family) -> dict | None:
    ...
    base = 4 * q + 1  # m = -floor(b phi) differs by up to about 1.62 q between sites, so the offset must exceed q
    keys_seen = []
    ...
    for lo in range(0, fam.count, chunk):
        ...
        for axis in range(dim):
            ...
            axis_id = (mval[bt] - mval[bs] + 2 * q) * base + (bt - bs + 2 * q)
    ...
    dm_all = np.zeros((len(distinct), dim), dtype=np.int64)
    db_all = np.zeros((len(distinct), dim), dtype=np.int64)
    rem = distinct.copy()
    for axis in range(dim - 1, -1, -1):
        axis_id = rem % (base * base)
        rem //= base * base
        dm_all[:, axis] = axis_id // base - 2 * q
        db_all[:, axis] = axis_id % base - 2 * q
    directions = exact_direction_count(dm_all, db_all)
    powers = acc.powers()
