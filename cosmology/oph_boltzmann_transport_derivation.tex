...
transport follows on an Observer-Patch Holography (OPH) branch that takes as
premises a geometry, photon refinement convergence, an electromagnetic vertex,
a polarized collision matrix, the electron mass, and a recombination history;
none of these is derived in this note. The reduction
gives the linear multipole hierarchy, acoustic and diffusion approximations,
and a line-of-sight transfer formula in a common brightness convention. It
also defines repair-exchange and collar-stress interfaces for conditional
extensions of the standard transport system.
\end{abstract}
...
The source Gram construction supplies a rank-three spatial carrier and an
algebraic Lorentzian extension. A nonzero closed convex pointed displacement
cone is future or past Lorentzian when the proper icosahedral rotations and
all rapidities of one boost axis preserve it; a time orientation fixes the
future cone. This requires operational covariance of histories, interventions
and clock readouts, beyond the algebraic frame action. It does not supply
the smooth metric assumed here. This branch requires physically identified
...
realizes a flat causal-order and count-volume limit using conservative
Fibonacci records, complete local neighbour reads and a declared layer
duration. Complete authenticated ancestry and interval anchors also give
the retrospective clock $(|I|/|J|)^{1/4}$. For interior timelike diamonds
and a nondegenerate reference interval $J$ in the same history, it converges
to the proper-time ratio without coordinates, timestamps or density in its
definition. The supplied event law and relative reference unit do not
calibrate the common physical clock required by the Einstein,
electromagnetic and cosmological transfer assumptions.
...
\(e^{-tL/2}\), and stationary density has covariance
...
screen-microphysics companion and preserved in
...
CMB transport calculation remain additional physical inputs.
...
\item P.-L. Lions and T. Paul, \emph{Sur les mesures de Wigner}, Rev. Mat.
Iberoam. 9, 553 (1993).
\item S. Bashinsky and U. Seljak, \emph{Signatures of Relativistic Neutrinos
